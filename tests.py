...
class Tests(unittest.TestCase):
    ...
    def test_reset_visited1(self):
        num_cols = 16
        num_rows = 16
        m1 = Maze(0, 0, num_rows, num_cols, 10, 10)
        test = m1._cells[0][0].visited
        expected = False
        self.assertEqual(test, expected)

    def test_reset_visited2(self):
        num_cols = 16
        num_rows = 16
        m1 = Maze(0, 0, num_rows, num_cols, 10, 10)
        test = m1._cells[0][0].visited
    ...
    def test_reset_visited3(self):
        num_cols = 16
        num_rows = 16
        m1 = Maze(0, 0, num_rows, num_cols, 10, 10)
        test = m1._cells[num_cols//2][num_rows//2].visited
        expected = False
        self.assertEqual(test, expected)
